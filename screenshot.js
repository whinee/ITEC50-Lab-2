...
let texContent = `% Automatically generated Lighthouse report for all pages\n\n`;

for (const file of htmlFiles) {
    const url = `http://localhost:${PORT}/${file}`;
    const result = await runLighthouse(url);

    texContent += `\\section{\\texttt{${file}}}
...
const outputFile = "lighthouse_reports.tex";
fs.mkdirSync(path.dirname(outputFile), { recursive: true });
fs.writeFileSync(outputFile, texContent);

console.log("LaTeX file created:", outputFile);
